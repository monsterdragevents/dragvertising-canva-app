...
class Conversion {
    static toLab(color) {
        if (color instanceof _color.RgbColor) {
            const xyz = Conversion.rgbToXyz(color);
            return Conversion.xyzToLab(xyz);
        } else if (color instanceof _color.XyzColor) return Conversion.xyzToLab(color);
        else if (color instanceof _color.HclColor) return Conversion.hclToLab(color);
        else if (color instanceof _color.HslColor) {
    ...
    static toRgb(color) {
        if (color instanceof _color.XyzColor) return Conversion.xyzToRgb(color);
        else if (color instanceof _color.LabColor) {
            const xyz = Conversion.labToXyz(color);
            return Conversion.xyzToRgb(xyz);
        } else if (color instanceof _color.HclColor) {
            const lab = Conversion.hclToLab(color);
            const xyz = Conversion.labToXyz(lab);
            return Conversion.xyzToRgb(xyz);
        } else if (color instanceof _color.HslColor) return Conversion.hslToRgb(color);
    ...
    static toHcl(color) {
        ...
        } else if (color instanceof _color.LabColor) return Conversion.labToHcl(color);
        else if (color instanceof _color.XyzColor) {
    ...
    static toHsl(color) {
        if (color instanceof _color.RgbColor) return Conversion.rgbToHsl(color);
        else if (color instanceof _color.HclColor) {
    ...
    static toXyz(color) {
        if (color instanceof _color.RgbColor) return Conversion.rgbToXyz(color);
        else if (color instanceof _color.LabColor) return Conversion.labToXyz(color);
        else if (color instanceof _color.HclColor) {
            const lab = Conversion.hclToLab(color);
    ...
    static labToXyz(color) {
        ...
        y *= D50.y;
        z *= D50.z;
        return new _color.XyzColor(x, y, z);
    }
    static xyzToLab(color) {
        ...
        const a = 500 * (fx - fy);
        const b = 200 * (fy - fz);
        return new _color.LabColor(l, a, b);
    }
    ...
    static rgbToXyz(color) {
        ...
        return new _color.XyzColor(x * 100, y * 100, z * 100);
    }
    static hexToRgb(c) {
        _preconditions.Preconditions.checkArgument(/^#[0-9a-f]{6}$/.test(c), 'non-compliant hex code');
        return _color.RgbColor.fromHexString(c);
    }
}
