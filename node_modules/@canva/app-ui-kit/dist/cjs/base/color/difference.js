...
const _lightness = require("./lightness");
const _normalize = require("./normalize");
class Difference {
    static de2000(color1, color2, kL = 1, kC = 1, kH = 1) {
        _preconditions.Preconditions.checkArgument(kL > 0);
        _preconditions.Preconditions.checkArgument(kC > 0);
        _preconditions.Preconditions.checkArgument(kH > 0);
        const { l: l1, a: a1, b: b1 } = _conversion.Conversion.toLab(color1);
        const { l: l2, a: a2, b: b2 } = _conversion.Conversion.toLab(color2);
        ...
        const h1Prime = (0, _normalize.normalizeHue)(radiansToDegrees(Math.atan2(b1, a1Prime)));
        const h2Prime = (0, _normalize.normalizeHue)(radiansToDegrees(Math.atan2(b2, a2Prime)));
    ...
    static wcagContrast(c1, c2) {
        const l1 = _lightness.Lightness.relativeLuminance(c1);
        const l2 = _lightness.Lightness.relativeLuminance(c2);
        return l1 > l2 ? (l1 + 0.05) / (l2 + 0.05) : (l2 + 0.05) / (l1 + 0.05);
    }
    static apcaContrast(foreground, background) {
        const luminanceForApca = (color)=>{
            const gamma = 2.4;
            const { r, g, b } = _conversion.Conversion.toRgb(color);
